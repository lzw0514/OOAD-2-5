...
import cn.edu.xmu.oomall.comment.controller.dto.OrderInfoDto;
import cn.edu.xmu.oomall.comment.dao.bo.CouponAct;
import cn.edu.xmu.oomall.comment.dao.bo.comment;
import cn.edu.xmu.oomall.comment.model.strategy.Item;
import cn.edu.xmu.oomall.comment.service.CouponActService;
...
public class CouponActController {
    ...
    public ReturnObject findAllOnlineCouponActivities(@RequestParam(required = false) Long shopId,
        ...
        List<CouponAct> acts = this.couponActService.retrieveValidByShopIdAndcommentIdAndTime(shopId, commentId, beginTime, endTime, page, pageSize);
     ...
     *
     */
    @GetMapping("/couponactivities/{id}/comments")
    @Transactional(propagation = Propagation.REQUIRED)
    public ReturnObject findCouponActcommentById(@PathVariable Long id,
                                                          @RequestParam(required = false, defaultValue = "1") Integer page,
                                                          @RequestParam(required = false, defaultValue = "10") Integer pageSize) {

        List<comment> comments = this.couponActService.retrievecomment(id);
        List<SimplecommentVo> dtos = comments.stream().map(comment -> new SimplecommentVo(comment)).collect(Collectors.toList());
