...
public class JTAdaptor implements LogisticsAdaptor {
    ...
    @Override
    public PostCreatePackageAdaptorDto createPackage(Account account, Express express) {

        Logistics logistics = account.getLogistics();
        if (logistics == null) {
...
//param.setServiceType();
        ...
        param.setCustomerCode(account.getAccount());
    ...
    @Override
    public Express getPackage(Account account, String billCode) {
        Logistics logistics = account.getLogistics();
        if (logistics == null) {
            throw new IllegalArgumentException("create package:logistics is null");
        }
        ...
        Express express = expressDao.findByBillCode(account.getShopId(), billCode);
    ...
    @Override
    public void cancelPackage(Account account, Express express){
        Logistics logistics = account.getLogistics();
        if (logistics == null) {
            throw new IllegalArgumentException("create package:logistics is null");
        }
    ...
    @Override
    public void sendPackage(Account account, String billCode, String orderId) {
        Express express = expressDao.findByBillCode(account.getShopId(), billCode);
        //在create方法默认传null
        createPackage(account,express);
    }
